...
//connecting buttons from GUI to functions
  g_signal_connect(connectBtn, "clicked", G_CALLBACK(gui_show_password_entry), NULL);
  g_signal_connect(unlockBtn, "clicked", G_CALLBACK(vnc_connect), NULL);
  ...
  g_signal_connect(vnc, "vnc-auth-failure", G_CALLBACK(gui_show_password_entry),NULL);
